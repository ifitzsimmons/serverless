...
module.exports = {
  async checkForChanges() {
    ...
    const objMetadata = await this.getObjectMetadata(objs);
    const lastModifiedDate = await this.getFunctionsEarliestLastModifiedDate();
    ...
    const response = await this.checkLogGroupSubscriptionFilterResourceLimitExceeded();
    return response;
  },
  ...
  async getMostRecentObjects() {
    ...
    let result;
    try {
      result = await this.provider.request('S3', 'listObjectsV2', params);
    } catch (reason) {
  ...
  async getFunctionsEarliestLastModifiedDate() {
    ...
    const results = await Promise.all(getFunctionResults);
  ...
  async getObjectMetadata(objects) {
    ...
    if (objects && objects.length) {
      const headObjectObjects = await objects.map(obj =>
      ...
      response = await Promise.all(headObjectObjects);
    }
  ...
  async checkIfDeploymentIsNecessary(objects, funcLastModifiedDate) {
    if (objects && objects.length) {
      ...
      const localHashes = await Promise.all(zipFileHashesPromises);
  ...
  async checkLogGroupSubscriptionFilterResourceLimitExceeded() {
    ...
    const returnPromises = this.serverless.service.getAllFunctions().map(functionName => {
    ...
    const allFunctions = await Promise.all(returnPromises);
    return allFunctions;
  },
  ...
  async fixLogGroupSubscriptionFilters(params) {
    ...
    let response;
    try {
      response = await cloudWatchLogsSdk.describeSubscriptionFilters({ logGroupName }).promise();
    } catch {
      /*
    ...
    */
    const deleteFilterResponse = await cloudWatchLogsSdk
